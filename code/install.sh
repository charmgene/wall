...
check_status() {
    ...
    if [[ $is_wget ]]; then
        ...
        [[ ! $is_fail ]] && {
            ...
            # sh download disabled for security
...
main() {
    ...
    # if wget installed. download core, jq, get ip (sh download disabled for security)
    [[ $is_wget ]] && {
        [[ ! $is_core_file ]] && download core &
        # sh download disabled - always use local install
        # [[ ! $local_install ]] && download sh &
        [[ $jq_not_found ]] && download jq &
        get_ip
    }
    # Force local install mode for security
    local_install=1
