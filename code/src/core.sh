    ...
    if [[ $is_use_tls ]]; then
        if [[ ! $is_no_auto_tls && ! $is_caddy && ! $is_gen ]]; then
            ...
            [[ $(is_test port_used 80) || $(is_test port_used 443) ]] && {
                ...
                msg "\e[41m no-auto-tls 帮助(help)\e[0m: 请参阅 v2ray no-auto-tls 文档\n"
...
get() {
    ...
    install-caddy)
        ...
        ;;
    reinstall)
        # Reinstall uses local files only for security
        is_install_sh=$(cat $is_sh_dir/install.sh)
        uninstall
        # Force local install to prevent downloading from 3rd party repo
        bash <<<$is_install_sh -l
        ;;
    test-run)
...
info() {
    ...
    tcp | kcp | quic)
        ...
        is_vmess_url=$(jq -c '{v:2,ps:'\"v2ray-${net}-$is_addr\"',add:'\"$is_addr\"',port:'\"$port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',type:'\"$header_type\"',path:'\"$kcp_seed\"'}' <<<{})
    ...
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#v2ray-$net-${is_addr}"
    ...
    ws | h2 | grpc)
        ...
        [[ $is_protocol == 'vmess' ]] && {
            is_vmess_url=$(jq -c '{v:2,ps:'\"v2ray-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        } || {
            [[ $is_trojan ]] && {
            ...
            is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&${is_url_path}=$(sed 's#/#%2F#g' <<<$path)#v2ray-$net-$host"
        }
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$is_addr:$port?encryption=none&security=reality&flow=xtls-rprx-vision&type=tcp&sni=$is_servername&pbk=$is_public_key&fp=ios#v2ray-$net-$is_addr"
        ;;
    door)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#v2ray-$net-${is_addr}"
        ;;
    http)
        is_can_change=(0 1)
        is_info_show=(0 1 2)
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装请使用 $is_core help 查看帮助"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
    ...
    if [[ $is_no_auto_tls ]]; then
        ...
        msg "\e[41m帮助(help)\e[0m: 请参阅 $is_core no-auto-tls 文档"
    fi
    footer_msg
}
...
url_qr() {
    ...
    if [[ $is_url ]]; then
        [[ $1 == 'url' ]] && {
            ...
            link="https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${is_url}"
...
update() {
    ...
    1 | core | $is_core)
        ...
        is_update_repo=$wall_repo
        ;;
    2 | sh)
        # Script updates disabled for security - prevents downloading from 3rd party repo
        warn "脚本更新已禁用, 无法从网络更新脚本 (Script updates disabled for security)"
        msg "如需更新脚本, 请手动下载并检查代码后使用本地安装"
        msg "To update, manually download and inspect the code, then use local install.\n"
        return
        ;;
    3 | caddy)
        ...
        ;;
    *)
        err "无法识别 ($1), 请使用: $is_core update [core | dat | caddy] [ver]"
        ;;
    esac
    [[ $2 ]] && is_new_ver=v${2#v}
    ...
    msg "$(_green 请查看更新说明: https://github.com/${wall_repo}/releases/tag/$is_new_ver)\n"
...
is_main_menu() {
    ...
    5)
        ...
        ;;
    6)
        # Script update option removed for security
        is_tmp_list=("更新$is_core_name")
        [[ $is_caddy ]] && is_tmp_list+=("更新Caddy")
        ask list is_do_update null "\n请选择更新:\n"
        # Map selection: 1=core, 2=caddy (skip sh which was option 2)
        [[ $REPLY == '1' ]] && update core
        [[ $REPLY == '2' ]] && update caddy
        ;;
    7)
        uninstall
        ;;
...
main() {
    ...
    u | up | update | U | update.sh)
        ...
        # Script update commands disabled for security
        [[ $1 == 'U' || $1 == 'update.sh' || $is_update_name == 'sh' ]] && {
            warn "脚本更新已禁用 (Script updates disabled for security)"
            msg "如需更新脚本, 请手动下载并检查代码后使用本地安装\n"
            return
        }
        if [[ $2 == 'dat' ]]; then
